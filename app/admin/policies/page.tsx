...
const ALL = "all";
const POLICY_STATUSES = ["Active", "Processing", "Expired"] as const;
...
export default function AdminPoliciesPage() {
  ...
  const rows = useMemo(() => {
    ...
    return adminPoliciesByRecency().filter((row) => {
      const matchesQuery =
        !query ||
  ...
  }, [activeTab, searchQuery, status]);

  const total = adminPoliciesByRecency().length;
        ...
        <div className="border border-[#F3F4F6] bg-white p-4 rounded-[8px] space-y-4">
          <div>
            ...
            <p className="text-sm text-[#6B7280] mt-0.5">
              Manage and monitor policies across all customers
            </p>
          </div>
          ...
          <CardContent className="p-4 space-y-4">
            ...
            <div className="flex flex-col gap-3 border-b border-[#E5E7EB] pb-4 lg:flex-row lg:items-center">
              <div className="relative w-full lg:max-w-[380px]">
                ...
                <Input
                  placeholder="Search by customer, policy number or product"
                  value={searchQuery}
            ...
            <div className="overflow-x-auto">
              <Table className="border border-[#F3F4F6] !rounded-[8px] min-w-[980px]">
                <TableHeader>
                ...
                <TableBody>
                  {rows.length === 0 ? (
                    ...
                    rows.map((row) => (
                      <TableRow
                        key={row.policyNumber ?? `${row.customer}-${row.datePurchased}`}
                        className="hover:bg-gray-50 transition-colors"
                      >
                        <TableCell className="py-3.5 pl-5 text-sm font-semibold text-[#AF060D]">
                          {row.policyNumber ?? (
                        ...
                        <TableCell className="py-3.5 text-sm font-semibold text-[#374151] whitespace-nowrap">
                          {row.customer}
                        </TableCell>
                        ...
                        <TableCell className="py-3.5 pr-5">
                          <Button
                            ...
                            onClick={() =>
                              router.push(`/admin/policies/${row.policyNumber}`)
                            }
                          >
