...
import { useFamilyBenefitsPlanStore } from "@/lib/store/lifeInsuranceStore";
...
function getPlanLabel(premium: string): string {
  if (premium === "25000") return "Premium Plan";
  if (premium === "15000") return "Standard Plan";
  return "Family Benefits Plan";
}

export default function FamilyBenefitsPlanPolicyPage() {
  const {
    currentStep,
    setStep,
    formData,
  ...
  const handleBackToHome = () => {
    reset();
    window.location.href = "/life-and-family-insurance/family-benefits-plan";
  };
      ...
      <section className="bg-[#fdfdfd] pb-16">
        <div className="mx-auto max-w-[1440px] px-4 lg:px-20">
          ...
          <div className="transition-all duration-300 max-w-[920px] mx-auto mt-8">
            ...
            {currentStep === 4 && (
              <Step3ReviewPay
                summaryRows={[
                  ...
                  { label: "Product:", value: getPlanLabel(formData.premiumAmount) },
                  {
            ...
            {currentStep === 5 && (
              <Step4PolicyDocument
                policyNumber="FBP-09795170"
                fullName={
                  `${formData.firstName} ${formData.lastname}`.trim() ||
                  "Policy Holder"
                }
                insuranceType="Family Benefits Plan"
